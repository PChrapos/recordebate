...
export const get: RequestHandler<{ name: string }> = async ({ params }) => {
    ...
    if (!model || !(await getRecordedModels()).includes(model)) return {
        status: 400
    }
    const videos = await getVideos(model)
...
export const del: RequestHandler<{ name: string }> = async ({ request, params: { name } }) => {
    ...
    if (!await deleteVideo(name, params.video)) return { status: 404 }
    return { status: 200 }
}
...
export const post: RequestHandler<{ name: string }> = async ({ request, params: { name } }) => {
    ...
    if (!await renameVideo(name, params.video, params.newName)) return { status: 404 }
    return { status: 200 }
}